...
TH1F ** rebin_501a()
{
    ...
    TRandom3 random;    
    TFile * outfile = TFile::Open("outfile_bc501a.root","RECREATE");
    ...
    for(int i=0; i<nFiles[0]; i++) {
        ...
        //hists0[i] = new TH1F(Form("hists0_%d",i),Form("hists0_%d",i),5010,-10,5000);
        ...
        for(int j=0; j<nEntries; j++) {
            ...
            //if(val[5]>tofCutLow0[i] && val[5]<tofCutHigh0[i]*1.1 && val[1]>10) hists0[i]->Fill(fillval);
            if(val[5]>tofCutLow0[i] && val[5]<tofCutHigh0[i] && val[1]>10 && fillval<phCutHigh0[i]*1.19) hists0[i]->Fill(fillval);
        }   
        for(int k=0; k<hists0[i]->FindBin(1); k++) hists0[i]->SetBinContent(k,0);
        outfile->cd();
        hists0[i]->Write(Form("ProtonCal%d",i));
        std::cout << " \t written to ProtonCal" << i << std::endl;
    }    
    ...
    for(int i=0; i<nFiles[1]; i++) {
        ...
        //hists1[i] = new TH1F(Form("hists1_%d",i),Form("hists1_%d",i),5010,-10,5000);
        ...
        for(int j=0; j<nEntries; j++) {
            ...
            //if(val[5]>tofCutLow1[i] && val[5]<tofCutHigh1[i] && val[1]>10) hists1[i]->Fill(fillval);
        }   
        for(int k=0; k<hists1[i]->FindBin(1); k++) hists1[i]->SetBinContent(k,0);
        outfile->cd();
        hists1[i]->Write(Form("ProtonCal%d",i+nFiles[0]));
        std::cout << " \t written to ProtonCal" << i+nFiles[0] << std::endl;
    }    
    ...
    //hists2[0] = new TH1F(Form("hists2_%d",0),Form("hists2_%d",0),5010,-10,5000);
    ...
    for(int j=0; j<nEntries; j++) {
        ...
        //if(val[5]>tofCutLow2 && val[5]<tofCutHigh2 && val[1]>10) hists2[0]->Fill(fillval);
    ...
    outfile->cd();
    hists2[0]->Write(Form("ProtonCal%d",nFiles[0]+nFiles[1]));
    std::cout << " \t written to ProtonCal" << nFiles[0]+nFiles[1] << std::endl;
    ...
    for(int i=0; i<=nFiles[3]; i++) {
        ...
        //hists3[i] = new TH1F(Form("hists3_%d",i),Form("hists3_%d",i),5010,-10,5000);
        ...
        outfile->cd();
        hists3[i]->Write(Form("ProtonCal%d",i+nFiles[0]+nFiles[1]+nFiles[2]));
        std::cout << " \t written to ProtonCal" << i+nFiles[0]+nFiles[1]+nFiles[2] << std::endl;
    }    
